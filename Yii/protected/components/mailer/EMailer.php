...
class EMailer {
	...
	/**
	 * The path to the directory where the view for getView is stored.
	 * Must not
	 * have ending dot.
	 *
	 * @var string
	...
	/**
	 * The path to the directory where the layout for getView is stored.
	 * Must
	 * not have ending dot.
	 *
	 * @var string
	...
	/**
	 * Constructor.
	 * Here the instance of PHPMailer is created.
	 */
	...
	public function getView($view, $vars = array(), $layout = null) {
		$body = Yii::app ()->controller->renderPartial ( $this->pathViews . '.' . $view, array_merge ( $vars, array (
				'content' => $this->_myMailer 
		) ), true );
		if ($layout === null) {
			$this->_myMailer->Body = $body;
		} else {
			$this->_myMailer->Body = Yii::app ()->controller->renderPartial ( $this->pathLayouts . '.' . $layout, array (
					'content' => $body 
			), true );
		}
	}
